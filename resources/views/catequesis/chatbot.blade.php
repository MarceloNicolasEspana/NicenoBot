...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @php($useVue = ($chatUi ?? 'vue') === 'vue')

        @if (file_exists(public_path('build/manifest.json')) || file_exists(public_path('hot')))
            @if ($useVue)
                @vite(['resources/css/app.css', 'resources/js/nicenito-chat.js'])
            @else
    ...
    <body class="min-h-screen bg-[var(--lp-bg)] text-[var(--niceno-ink)]">
        @if ($useVue)
            {{-- Interfaz Vue moderna. Datos iniciales seguros (sin PIN, código,
                 nombre completo ni IDs internos). El backend es la fuente de
                 verdad: el front nunca envía participant_id. --}}
            <div
                id="nicenito-app"
        ...
        @else
            {{-- ============================================================
                 Interfaz legacy (Blade + JavaScript). Fallback temporal,
                 activable con NICENITO_CHAT_UI=legacy. No eliminar hasta
                 validar la interfaz Vue en producción.
                 ============================================================ --}}
